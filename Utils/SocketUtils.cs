...
namespace Utils
{
    public static class SocketUtils
    {
        ...
        public static bool Receive(TcpClient client, uint maxSize, out byte[] buffer)
        {
        ...
        /// <returns></returns>
        public static bool Receive(Socket client, uint maxSize, out byte[] buffer)
        {
            buffer = new byte[0];
            int offset = 0;
            if (client.Connected)
        ...
        public static void Send(Socket client, byte[] data, int offset, int size)
        {
            if (client.Connected)
            {
                try
